...
char *make_block (int block_size) {
   ...
   */
    //printf("make block");
    char *blocks = (char *)malloc(sizeof(char)*block_size);
...
Set *make_sets (int set_count, int line_count, int block_size) {
   ...
   */
    ...
    ((set[i])).lru_queue=malloc(sizeof(LRUNode));
  }
...
Cache *make_cache (int set_bits, int line_count, int block_bits) {
  ...
  //printf("make cache");
     ...
     //cache->sets=make_sets(set_bits,line_count,block_bits); 
     (*cache).line_count=line_count;
     (*cache).block_size=(int)exp2((double)block_bits);
     (*cache).set_bits=set_bits;
     (*cache).block_bits=block_bits;
     cache->set_count=(int)exp2((double)set_bits);
...
AccessResult cache_access (Cache *cache, TraceLine *trace_line) {
  ...
  // printf("Did not finish executing\n"); 
  // Get the set:
  Set *set   = &cache->sets[s];

  // Get the line:
  LRUResult result;
  //printf("prior to fetch\n");
  ...
  if (result.access != HIT) {
   // printf("the hit line\n");
    for (int i = 0; i < cache->block_size; i++) {
     // printf("after hit loop\n");
      line->accessed[i] = 0;
    //  printf("accessing the line\n");
    }
  }
  //printf("Hey almost done\n");
